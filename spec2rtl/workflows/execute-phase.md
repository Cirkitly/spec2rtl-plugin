...
| s2r-verifier | sonnet | sonnet | haiku |
...
if [ "$BRANCHING_STRATEGY" = "phase" ]; then
  # Get phase name from directory (e.g., "03-mem-ctrl" → "mem-ctrl")
...
**No dependency analysis needed.** Wave numbers are pre-computed during `/s2r:plan-phase`.
...
1. **Describe what's being built (BEFORE spawning):**
   ...
   **Examples:**
   ...
   - Good: "Implement AXI4 Interconnect logic and Address decoding. Required before CPU can access Peripherals."
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   Include steps for writing RTL, writing testbenches, running simulations, and checking metrics where appropriate.
   ...
   @~/.claude/spec2rtl/references/vdd.md
   </execution_context>
   ...
   - [ ] RTL implemented and linted (if applicable)
   - [ ] Testbenches updated/created
   - [ ] Simulations run and passing
   - [ ] Timing/Area metrics checked
...
3. **Report completion and what was built:**
   ...
   **Examples:**
   ...
   - Good: "DDR4 Controller Initialized — Calibration FSM passes simulation. Memory Map (Wave 3) can now rely on valid address range."
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="s2r-executor", model="{executor_model}")
   ```
...
Check must_haves against actual codebase (RTL/TB). Create VERIFICATION.md.
Check coverage reports and simulation logs.
Verify what actually exists in the code and if timing/area metrics are met.",
  subagent_type="s2r-uvm-engineer",
...
| `gaps_found` | Present gap summary, offer `/s2r:plan-phase {phase} --gaps` |
...
- `/s2r:verify-work {X}` — manual testing before planning
```

User runs `/s2r:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/s2r:execute-phase {X} --gaps-only`
...
1. Run `/s2r:execute-phase {phase}` again
