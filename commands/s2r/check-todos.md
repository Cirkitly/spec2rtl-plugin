...
Todos are captured during work sessions with /s2r:add-todo.

---

Would you like to:

1. Continue with current phase (/s2r:progress)
2. Add a todo now (/s2r:add-todo)
```
...
- `/s2r:check-todos` → show all
- `/s2r:check-todos api` → filter to area:api only
</step>
...
- `/s2r:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /s2r:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/s2r:add-phase [description from todo]`
...
- Don't create plans from this command — route to /s2r:plan-phase or /s2r:add-phase
</anti_patterns>
