...
Example: `/s2r:insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /s2r:insert-phase <after> <description>"
  echo "Example: /s2r:insert-phase 72 Fix critical auth bug"
  exit 1
fi
```
...
2. Insert new phase heading with (INSERTED) marker:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /s2r:plan-phase {decimal_phase} to break down)
...
- Don't use this for planned work at end of milestone (use /s2r:add-phase)
