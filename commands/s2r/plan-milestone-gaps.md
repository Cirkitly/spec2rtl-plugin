...
<objective>
Create all phases necessary to close gaps identified by `/s2r:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/s2r:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns s2r-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/s2r:audit-milestone` first.
```
...
**Also available:**
- `/s2r:execute-phase {N}` — if plans already exist
...
`/s2r:audit-milestone` — re-audit to verify gaps closed
`/s2r:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/s2r:plan-phase` next
</success_criteria>
