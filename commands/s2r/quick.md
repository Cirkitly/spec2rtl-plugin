---
name: gsd:quick
description: Execute a quick task with Spec2RTL guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
allowed-tools:
  - Read
  - Write
...
<objective>
Execute small, ad-hoc tasks with Spec2RTL guarantees (atomic commits, STATE.md tracking) while skipping optional agents (research, plan-checker, verifier).

Quick mode is the same system with a shorter path:
- Spawns s2r-planner (quick mode) + s2r-executor(s)
- Skips s2r-phase-researcher, s2r-plan-checker, s2r-verifier
...
<execution_context>
Orchestration is inline - no separate workflow file. Quick mode is deliberately simpler than full Spec2RTL.
</execution_context>
...
Check that an active Spec2RTL project exists:

```bash
if [ ! -f .planning/ROADMAP.md ]; then
  echo "Quick mode requires an active project with ROADMAP.md."
  echo "Run /s2r:new-project first."
  exit 1
fi
```
...
Spawn s2r-planner with quick mode context:
...
Spawn s2r-executor with plan reference:
