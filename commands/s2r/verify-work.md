...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /s2r:execute-phase
</objective>

<execution_context>
@~/.claude/spec2rtl/workflows/verify-work.md
@~/.claude/spec2rtl/templates/UAT.md
</execution_context>
...
8. If issues found:
   ...
   - Spawn s2r-planner in --gaps mode to create fix plans
   - Spawn s2r-plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/s2r:execute-phase`
</process>
...
- /s2r:execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- /s2r:complete-milestone — skip audit, archive directly
...
- /s2r:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /s2r:plan-phase {Z} --gaps — retry fix planning with guidance
- /s2r:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: s2r-planner creates fix plans from diagnosed gaps
- [ ] If issues: s2r-plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/s2r:execute-phase` when complete
</success_criteria>
